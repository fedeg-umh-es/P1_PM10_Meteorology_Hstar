...
\textbf{Main findings.}
At Madrid (lag-1 autocorrelation $\rho_1 \approx 0.96$), adding meteorology
extends $H^*_{\text{strict}}$ (longest contiguous positive-skill run) from
9\,h to 17\,h ($\Delta H^* = +8$\,h), a gain that is statistically significant
at $h = 12$ ($p = 0.012$, DM-HLN).  At the eight Irish stations
($\rho_1 \approx 0.85$), the mean gain is only $+1.0$\,h, with lags-only
XGBoost already achieving near-maximum horizon skill.  We interpret this
contrast in terms of the local autocorrelation-persistence
...
and its concentration dynamics are shaped by the same boundary-layer and
...
boundary-layer ventilation frequency — is associated with both the skill of
data-driven forecasting models and the incremental value of explicit
meteorological covariates.  The study covers two contrasting European regimes (continental
...
urgent.  Because our meteorological inputs are retrospective observations rather
than origin-time forecasts, the results characterise a retrospective upper bound
and a testable hypothesis for where meteorological data pipelines are most likely
to help, rather than demonstrated operational value — a question of direct
